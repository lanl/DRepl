...
func sync(fid *Fid) error {
	if fid.s >= fid.e {
		return nil
	}

	start := uintptr(unsafe.Pointer(&fid.data[fid.s])) &^ (0xfff) // start address needs to be page-aligned
	end := uintptr(unsafe.Pointer(&fid.data[fid.e-1]))
...
func (*Ufs) Open(req *srv.Req) {
	...
	fid.data, e = syscall.Mmap(int(fid.file.Fd()), 0, int(fid.st.Size()), syscall.PROT_READ|syscall.PROT_WRITE, syscall.MAP_SHARED)
	if e != nil {
		req.RespondError(toError(e))
		return
	}
...
func (*Ufs) Write(req *srv.Req) {
	...
	if fid.data != nil && tc.Offset + uint64(len(tc.Data)) < uint64(len(fid.data)) {
		n = copy(fid.data[tc.Offset:], tc.Data)
//		e := sync(fid)
//		if e != nil {
//			req.RespondError(toError(e))
//			return
